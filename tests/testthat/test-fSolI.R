...
test_that('Zenith angle in fSolI',{
    expect_equal(solI$cosThzS, solI_expected$cosThzS, tolerance = 1e-4)
})

test_that('Azimuth angle in fSolI',{
    expect_equal(solI$AzS, solI_expected$AzS, tolerance = 1e-4)
})

test_that('Solar altitude angle in fSolI',{
    expect_equal(solI$AlS, solI_expected$AlS, tolerance = 1e-4)
})

test_that('Extraterrestrial irradiance in fSolI',{
    expect_equal(solI$Bo0, solI_expected$Bo0, tolerance = 1e-4)
})
